...
Current artifacts use output schema `3.1.0` with explicit finding-budget status. Preserved schema `3.0.0` artifacts remain loadable and are validated against their own hashes, identities, and recorded rule sets instead of the current registry. `compare-experiments` rejects mismatched corpus/split/sample/ground-truth/threshold identities, reports B−A paired changes for compatible runs, warns about rule-pack/version/provenance differences, and withholds latency deltas unless timing boundary and runtime environment match. Schemas older than `3.0.0` are rejected rather than silently upgraded. See the [research protocol](research-protocol.md) and complete the [experiment-plan template](experiment-plan-template.md) before a future holdout run.
...
Development corpus version 1.0.0 currently yields TP 37, TN 36, FP 4, and FN 3: accuracy 91.25%, precision 90.24%, recall 92.50%, F1 91.36%, FPR 10.00%, FNR 7.50%, and specificity 90.00%. The Day 5B relation-scoping correctness fixes retain this established development regression baseline without changing rules, thresholds, samples, or labels. These values are calculated by the evaluator and asserted as a regression baseline. They are not holdout accuracy and must be updated transparently if detector behavior or ground truth changes.
